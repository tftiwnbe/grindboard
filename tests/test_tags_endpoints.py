...
class TestListTags:
    ...
    async def test_lists_created_tags(
        ...
        # Create tags
        await client.post("/tags/?name=work", headers=auth_headers)
        await client.post("/tags/?name=personal", headers=auth_headers)
...
class TestCreateTag:
    ...
    async def test_creates_tag(self, client: AsyncClient, auth_headers: dict[str, str]):
        ...
        response = await client.post("/tags/?name=urgent", headers=auth_headers)
    ...
    async def test_prevents_duplicate_tags(
        ...
        # Create tag
        r1 = await client.post("/tags/?name=work", headers=auth_headers)
        tag1 = r1.json()

        # Try to create duplicate
        r2 = await client.post("/tags/?name=work", headers=auth_headers)
        tag2 = r2.json()
        ...
        list_response = await client.get("/tags/", headers=auth_headers)
...
class TestRenameTag:
    ...
    async def test_renames_tag(self, client: AsyncClient, auth_headers: dict[str, str]):
        ...
        create_response = await client.post(
            "/tags/?name=old_name", headers=auth_headers
        )
        tag_id = create_response.json()["id"]

        # Rename tag
        response = await client.put(
            f"/tags/{tag_id}?name=new_name", headers=auth_headers
        )
    ...
    async def test_merge_on_duplicate_name(
        ...
        # Create two tags
        r1 = await client.post("/tags/?name=tag1", headers=auth_headers)
        tag1_id = r1.json()["id"]

        r2 = await client.post("/tags/?name=tag2", headers=auth_headers)
        tag2_id = r2.json()["id"]

        # Create task and add tag1 to it
        task = await make_task(title="Test Task")
        await client.post(
            f"/tags/tasks/{task['id']}/tags/{tag1_id}", headers=auth_headers
        )

        # Rename tag2 to tag1 (should merge)
        response = await client.put(f"/tags/{tag2_id}?name=tag1", headers=auth_headers)
        assert response.status_code == 200
    ...
    async def test_rename_nonexistent_tag(
        ...
        response = await client.put("/tags/999999?name=new_name", headers=auth_headers)
...
class TestDeleteTag:
    ...
    async def test_deletes_tag(self, client: AsyncClient, auth_headers: dict[str, str]):
        ...
        # Create tag
        create_response = await client.post(
            "/tags/?name=to_delete", headers=auth_headers
        )
        tag_id = create_response.json()["id"]
    ...
    async def test_delete_removes_from_tasks(
        ...
        # Create tag and task
        tag_response = await client.post("/tags/?name=test_tag", headers=auth_headers)
        ...
        # Add tag to task
        await client.post(
            f"/tags/tasks/{task['id']}/tags/{tag_id}", headers=auth_headers
        )
        ...
        task_response = await client.get("/tasks/", headers=auth_headers)
    ...
    async def test_delete_nonexistent_tag(
        ...
        """Should return 404 for nonexistent tag."""
        response = await client.delete("/tags/999999", headers=auth_headers)
        assert response.status_code == 404


class TestAddTagToTask:
    """Tests for POST /tags/tasks/{task_id}/tags/{tag_id} endpoint."""
    ...
    async def test_adds_existing_tag_to_task(
        ...
        # Create tag and task
        tag_response = await client.post("/tags/?name=important", headers=auth_headers)
        tag_id = tag_response.json()["id"]
    ...
    async def test_idempotent_add(
        ...
        tag_response = await client.post("/tags/?name=test", headers=auth_headers)
        tag_id = tag_response.json()["id"]
    ...
    async def test_add_tag_to_nonexistent_task(
        ...
        response = await client.post(
            f"/tags/tasks/999999/tags/{tag_id}", headers=auth_headers
        )
        assert response.status_code == 404
    ...
    async def test_add_nonexistent_tag_to_task(
        ...
        response = await client.post(
            f"/tags/tasks/{task['id']}/tags/999999", headers=auth_headers
        )
        assert response.status_code == 404


class TestRemoveTagFromTask:
    """Tests for DELETE /tags/tasks/{task_id}/tags/{tag_id} endpoint."""
...
class TestCreateAndAddTag:
    """Tests for POST /tags/tasks/{task_id}/tags endpoint."""
    ...
    async def test_creates_and_adds_new_tag(
        ...
        response = await client.post(
            f"/tags/tasks/{task['id']}/tags?name=new_tag", headers=auth_headers
        )
    ...
    async def test_uses_existing_tag(
        ...
        # Create tag first
        tag_response = await client.post("/tags/?name=existing", headers=auth_headers)
        ...
        response = await client.post(
            f"/tags/tasks/{task['id']}/tags?name=existing", headers=auth_headers
        )
    ...
    async def test_create_and_add_to_nonexistent_task(
        ...
        """Should return 404 for nonexistent task."""
        response = await client.post(
            "/tags/tasks/999999/tags?name=test", headers=auth_headers
        )
        assert response.status_code == 404
...
class TestAuthentication:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("delete", "/tags/1"),
            ("post", "/tags/tasks/1/tags/1"),
            ("delete", "/tags/tasks/1/tags/1"),
            ("post", "/tags/tasks/1/tags?name=test"),
        ],
    )
